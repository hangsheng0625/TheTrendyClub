  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-title text-2xl sm:text-3xl mb-2">Order Management</h1>
      ...
      {orders.length === 0 ? (
        <div className="admin-card p-8 sm:p-12 text-center">
          <img src={assets.parcel_icon} alt="No orders" className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-4 opacity-50" />
          <h3 className="text-subtitle text-lg sm:text-xl mb-2">No Orders Yet</h3>
          <p className="text-body-secondary text-sm sm:text-base">Orders will appear here once customers start placing them.</p>
        </div>
      ) : (
        <div className="space-y-4 sm:space-y-6">
          {orders.map((order, index) => (
            <div key={index} className="admin-card p-4 sm:p-6 hover:shadow-lg transition-shadow">
              {/* Order Header */}
              <div className="flex flex-col gap-4 mb-4 sm:mb-6 pb-4 border-b border-gray-100">
                <div className="flex items-start sm:items-center gap-3">
                  <img src={assets.parcel_icon} alt="Order" className="w-6 h-6 sm:w-8 sm:h-8 mt-1 sm:mt-0" />
                  <div className="flex-1 min-w-0">
                    <h3 className="text-subtitle text-base sm:text-lg">Order #{order._id.slice(-8)}</h3>
              ...
              <div className="space-y-6 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-6">
                ...
                <div className="lg:col-span-2">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {order.items.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-3 sm:gap-4 p-2 sm:p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex-1 min-w-0">
                          <h5 className="text-body-primary font-medium text-sm sm:text-base truncate">{item.name}</h5>
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs sm:text-sm text-body-secondary mt-1">
                            ...
                            <span className="font-medium text-body-primary">{currency}{item.price}</span>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
                    <h4 className="text-body-primary font-medium mb-2 sm:mb-3 text-sm sm:text-base">Customer Details</h4>
                  ...
                  {/* Order Summary */}
                  <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
                    <h4 className="text-body-primary font-medium mb-2 sm:mb-3 text-sm sm:text-base">Order Summary</h4>
                    <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm">
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                        ...
                        <span className="text-base sm:text-lg font-bold text-body-primary">{currency}{order.amount}</span>
                      </div>
                    </div>
                  </div>
                </div>
