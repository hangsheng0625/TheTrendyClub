  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-title text-2xl sm:text-3xl mb-2">All Products List</h1>
        <p className="text-body-secondary">Manage your product inventory from here.</p>
      </div>
              ...
              <div className='admin-card p-4 md:p-2' key={index}>
                ...
                <div className='md:hidden space-y-3'>
                  <div className='flex items-start gap-4'>
                    <img className='w-16 h-16 object-cover rounded-lg border border-gray-200 flex-shrink-0' src={item.image[0]} alt="" />
                    <div className='flex-1 min-w-0'>
                      <h3 className='font-medium text-body-primary truncate'>{item.name}</h3>
                      <p className='text-sm text-body-secondary'>{item.category}</p>
                      <p className='text-lg font-semibold text-body-primary'>{currency}{item.price}</p>
                    </div>
                    <button 
                ...
                <div className='hidden md:grid grid-cols-[1fr_3fr_1fr_1fr_1fr] items-center gap-2 py-2 px-2 text-sm'>
                  <img className='w-12 h-12 object-cover rounded-lg border border-gray-200' src={item.image[0]} alt="" />
                  <p className='font-medium text-body-primary'>{item.name}</p>
                  <p className='text-body-secondary'>{item.category}</p>
                  <p className='font-semibold text-body-primary'>{currency}{item.price}</p>
                  <button 
                    onClick={()=>removeProduct(item._id)} 
